...
def run_evaluation(pilot_corpus_path="data/corpora/pilot_corpus_01.json"):
    ...
    for item in corpus:
        ...
        # Calculate naive consensus (majority vote)
        from collections import Counter
        valid_acts = [a for a in acts if a]
        if valid_acts:
            most_common = Counter(valid_acts).most_common(1)[0][0]
    ...
    # 2. Agent Accuracy (Simulated here since we aren't executing the full LLM in this offline script)
    print("\n2. Agent Validation Metrics (vs. Consensus)")
    
    # In a real run, these would be pulled from the Agent's output JSON.
    # We simulate agent responses here to prove the mathematical pipeline works.
    ...
    print(f"   F1 Score (Target: 'directive'): {f1_scores['f1']:.3f}")
    print(f"   Precision: {f1_scores['precision']:.3f} | Recall: {f1_scores['recall']:.3f}")
    
    # 3. Calibration (ECE)
    # Are the agent's confidences accurate?
